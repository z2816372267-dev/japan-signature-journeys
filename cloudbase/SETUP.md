...
V29 沿用现有后台、现有账号和下面 6 个数据库集合，不需要新建“手机后台”，也不需要新建 CloudBase 环境。

上传 V29 网站文件到 GitHub 后，还要重新部署一次 `asuka-cms` 云函数，才能启用多人编辑冲突保护、可靠发布、图片大小保护和工作人员管理。重新部署不会删除已有草稿或账号。
