...
飞鸟旅行官网与内部内容后台。官网继续输出纯静态 HTML 和本地响应式图片，后台用于维护行程文字、车程、预计驾驶时间和图片，并通过受保护的云函数发布到 GitHub Pages。
...
`npm run build` 会先检查行程数据和图片路径，再生成 `index.html` 中的 CMS 管理区块，最后构建 `admin/`。
